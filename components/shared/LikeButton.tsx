...
export const LikeButton = ({
  ...
  const { currentUserLikes, totalPostLikes, isLoading, incrementLikes } = usePostData(id, title);
        ...
        <Box position="relative">
          ...
          >
            <MotionBox
              ...
              initial="0"
            />
            <Icon as={BsHeartFill} w={5} h={5} position="relative" color={'white'} />
          </MotionFlex>
        </Box>
      </button>
      <Box fontSize="lg" fontWeight="semibold" color={linkColor}>
        {isLoading ? (
          <Spinner size="xs" speed="0.65s" emptyColor="gray.200" color={linkColor} />
        ) : (
